...
from app.models import Producto, Compra, Venta, DetalleVenta
    ...
    try:
        ...
        for compra in compras_vencer:
            ...
            if dias_restantes == 0:
                nivel = 'error'
                icono = '🚨'
                mensaje = f'Compra #{compra.idCompra} - {compra.Producto.nombre} VENCE HOY'
            elif dias_restantes == 1:
                nivel = 'warning'
                icono = '⚠️'
                mensaje = f'Compra #{compra.idCompra} - {compra.Producto.nombre} vence MAÑANA'
            else:
                nivel = 'info'
                icono = '📅'
                mensaje = f'Compra #{compra.idCompra} - {compra.Producto.nombre} vence en {dias_restantes} días'
